...
ユーザーから期間とジャンルを受け取る。不明な場合は AskUserQuestion で確認する。
...
Phase 1 完了時に推奨モード（Quick / Full）を提案し、ユーザーの承認または上書きを AskUserQuestion で確認してから Phase 2 へ進む。
...
| ソース | 読み込む内容 |
|--------|------------|
| `AGENTS.md` / `CLAUDE.md` | 技術スタック、アーキテクチャ方針、コーディング規約 |
| `Package.swift` / `build.gradle` 等 | 依存ライブラリとバージョン（インパクト分析の主要インプット） |
...
**検索戦略**:
1. ジャンルごとに 2〜3 回の WebSearch を実行（英語メイン + 日本語補完）
