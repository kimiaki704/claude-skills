...
| Phase 3 WebSearch 回数 | ジャンルあたり 1〜2 回 | ジャンルあたり 2〜3 回 |
...
**AskUserQuestion は最大2問に制限する（Progressive Disclosure）**:
...
|--------|-------|------|------------|
| `AGENTS.md` / `CLAUDE.md` | ✅ | ✅ | 技術スタック、アーキテクチャ方針 |
...
**検索戦略**:
1. ジャンルごとに Quick: 1〜2 回 / Full: 2〜3 回 の WebSearch を実行（英語メイン + 日本語補完）
2. `references/genre-search-patterns.md` の検索パターンを参考にする
3. ソースの優先順位: 公式リリースノート / changelog > 企業エンジニアブログ > 著名個人ブログ > GitHub trending

**WebSearch 上限**:
- 1実行の上限: ジャンル数 × Quick 2 回 / Full 3 回
- Claude.ai 環境（レートリミットが厳しい場合）: ジャンルあたり 1 回に制限し、「Claude.ai 環境のため検索回数を制限しています」とユーザーに明示する
