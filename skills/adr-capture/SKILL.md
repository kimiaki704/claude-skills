...
**決定前の相談の場合**: 選択肢を整理してトレードオフを分析し、決定を支援してから ADR を生成する。
...
**出力先の決定**（ユーザーへの事前確認なしで判断）:
- Claude Code 環境 + ADR ディレクトリが特定できる場合: Write で直接書き出す
- それ以外: テキスト出力（ユーザーが手動でコピー・保存）

### Step 4: 関連 ADR のリンク提案（オプション）

Claude Code 環境で既存 ADR が存在する場合のみ: Tags をもとに関連 ADR を自動検出し、リンクを提案する（強制ではなく提案にとどめる）。
...
2. 各 ADR のフロントマター（Status, Date, Tags）のみ読み取る（本文は読まない）
3. 以下の形式で出力:
