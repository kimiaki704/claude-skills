...
| worktree パス | `.claude/worktrees/[skill-name]` | `.claude/worktrees/lt-outline-builder` |
...
```bash
# worktree 作成（プロジェクト内 .claude/worktrees/ に配置）
mkdir -p .claude/worktrees
git worktree add .claude/worktrees/[skill-name] -b feat/[skill-name]

# ディレクトリ構造
mkdir -p .claude/worktrees/[skill-name]/skills/[skill-name]/references
```
...
```bash
# worktree 内でコミット
git -C .claude/worktrees/[skill-name] add skills/[skill-name]/ \
  .claude-plugin/marketplace.json .claude-plugin/plugin.json README.md
git -C .claude/worktrees/[skill-name] commit -m "feat: [skill-name] スキルを追加"

# プッシュ
git -C .claude/worktrees/[skill-name] push -u origin feat/[skill-name]

# PR 作成
cd .claude/worktrees/[skill-name] && gh pr create --draft \
...
```bash
git worktree remove .claude/worktrees/[skill-name]
...
| worktree が既に存在する | `git worktree list` で確認し、`.claude/worktrees/[skill-name]` の既存 worktree を使うか削除するか聞く |
